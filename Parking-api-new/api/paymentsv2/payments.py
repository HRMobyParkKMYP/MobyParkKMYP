...
async def create_payment(request: CreatePaymentRequest, authorization: Optional[str] = Header(None, alias="Authorization")):
    user = require_auth(authorization)
    try:
        payment = payment_utils.create_payment(
            user_id=user["id"],
            reservation_id=request.reservation_id,
            amount=request.amount,
            currency=request.currency,
...
@router.post("/payments/refund")
async def refund_payment(transaction: int, authorization: Optional[str] = Header(None, alias="Authorization")):
    require_auth(authorization)
    result = payment_utils.refund_payment(transaction)
    if not result:
        raise HTTPException(status_code=404, detail="Payment not found")
    return {"status": "success", "message": "Payment refunded"}
...
@router.put("/payments/{transaction}")
async def complete_payment(transaction: int, request: UpdatePaymentRequest, authorization: Optional[str] = Header(None, alias="Authorization")):
    require_auth(authorization)
    result = payment_utils.update_payment(transaction, request.status, request.paid_at)
    if not result:
        raise HTTPException(status_code=404, detail="Payment not found")
    return {"status": "success"}
...
@router.get("/payments")
async def get_payments(authorization: Optional[str] = Header(None, alias="Authorization")):
    user = require_auth(authorization)
    payments = payment_utils.get_payments(user["id"])
    return {"status": "success", "payments": payments}


@router.get("/payments/{username}")
async def get_user_payments(username: str):
    payments = payment_utils.get_user_payments(username)
    return {"status": "success", "payments": payments}
